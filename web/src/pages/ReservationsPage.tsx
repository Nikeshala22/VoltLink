      ...
      <div className="card mb-4 p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <FilterChips
            ...
            options={STATUS_FILTERS.map((status) => ({ value: status, label: status || 'All' }))}
          />
              ...
              {reservations.map((r) => (
                <li key={r.id} className="p-4">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-ink-500">
                    ...
                    <span className="numeric">{r.energyKwh} kWh</span>
                    <span
                      className={`badge ${
                        r.type === 'Injection'
                ...
                {reservations.map((r) => (
                  <tr key={r.id}>
                    <td className="font-medium text-ink-900">
                      <Link to={`/reservations/${r.id}`} className="transition-colors hover:text-brand-600">
                        {r.reservationNo}
                      </Link>
                    </td>
                    <td>
